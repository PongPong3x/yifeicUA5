...
let addScore = (quizTaker,quizId, score) => {
    ...
    .then(x => pool.query('insert into imagequiz.score(customer_id,quiz_id,score) values ($1,$2,$3)',[x.rows[0].customer_id,quizId,score]).then(x => x.rows));
...
let checkScore = (quiztaker,quizid) => {
    return pool.query('select customer_id from imagequiz.customer where customer_id = $1',[quiztaker])
    .then(x => pool.query('select score from imagequiz.score where (customer_id = $1 and quiz_id = $2)',[x.rows[0].customer_id,quizId]).then(x => x.rows));
}
