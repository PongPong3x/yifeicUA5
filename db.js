...
let addScore = (quizTaker,quizId, score) => {
    ...
    return pool.query('select customer_id from imagequiz.customer where customer_id = $1',[quizTaker.toLowerCase()])
    .then(x => pool.query('insert into imagequiz.score(customer_id,quiz_id,score,date) values ($1,$2,$3,$4)',[parseInt(x.rows[0]).customer_id,parseInt(quizId),score,1]).then(x => x.rows));
...
let checkScore = (quiztaker,quizid) => {
    return pool.query('select customer_id from imagequiz.customer where customer_id = $1',[quiztaker.toLowerCase()])
    .then(x => pool.query('select * from imagequiz.score where (customer_id = $1 and quiz_id = $2)',[parseInt(x.rows[0].customer_id),parseInt(quizid)]).then(x => x.rows));
}
